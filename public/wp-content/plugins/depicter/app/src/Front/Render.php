...
class Render
{
	...
	public function document( $documentID = 0, $args = [] ){
		...
		$args = $this->maybeEnqueueAssets( $documentID, $args );
	...
	protected function maybeEnqueueAssets( $documentID, $args = [] ){
		if ( ! did_action( 'depicter/after/enqueue/styles' ) ) {
			\Depicter::front()->assets()->enqueueStyles();

			if ( $args['isPrivilegedUser'] ) {
				\Depicter::front()->assets()->enqueueCustomGoogleFonts( $documentID );
			} else {
				// $args['loadStyleMode'] = 'inline';
				\Depicter::front()->assets()->enqueueCustomStyles( $documentID );
			}
		}
